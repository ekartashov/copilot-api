...
describe("getCopilotToken", () => {
  ...
  test("should fetch Copilot token successfully", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(new Response(JSON.stringify(mockTokenData), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    }))

    // Import dynamically to avoid module mock conflicts
    const { getCopilotToken } = await import("../../../src/services/github/get-copilot-token")
    const result = await getCopilotToken()

    expect(fetchSpy).toHaveBeenCalledTimes(1)
    expect(fetchSpy).toHaveBeenCalledWith("https://api.github.com/copilot_internal/v2/token", {
